...
5     print*,' Enter name of input frechet file'
      ...
      print*, 'Time taken to execute the section: ', elapsed_time, ' seconds. "Enter name of input frechet file"'
...
c     set up integration parameters and constants
      call system_clock(end_time)
      elapsed_time = real(end_time - start_time) / real(clock_rate)
      print*, 'Time taken to execute the section: ', elapsed_time, ' seconds. Spot B'
c
      rad(knot+1)=0.0
      rad(1)=1.0
      if (jcom .eq. 2) then
...
c        print*,'irec',irec
     ...
     &    nnsave(imodefnd),llsave(imodefnd),p
      ...
      call system_clock(end_time)
      elapsed_time = real(end_time - start_time) / real(clock_rate)
      print*, 'Time taken to execute the section: ', elapsed_time, ' seconds. Spot C'
...
c
     ...
     &             + buf(k2+j)*dbetah(ind+j)
          ...
          call system_clock(end_time)
          elapsed_time = real(end_time - start_time) / real(clock_rate)
          print*, 'Time taken to execute the section: ', elapsed_time, ' seconds. Spot D'
c
ccc write out
